...
export default function AboutUsClient({ data }: { data: WpPage }) {
    ...
    const missionInView = useInView(missionRef, { once: true, margin: "-120px" });
    const reasonInView = useInView(reasonRef, { once: true, margin: "-120px" });
    ...
    const missionSectionData = acf.mission_section?.find(item => item.acf_fc_layout === "mission_section_data") as any;
    const scheduleData = acf.mission_section?.find(item => item.acf_fc_layout === "schedule_meet") as any;
    const missionCardsRaw = acf.mission_section?.filter(item => item.acf_fc_layout === "mission_card") as any[];
        ...
        <main className="w-full overflow-hidden">
            ...
            <section ref={sectionRef} className="relative min-h-screen bg-black text-white flex items-center justify-center overflow-hidden">
                {heroData?.about_hero_image?.url ? (
                    ...
                    <div className="absolute inset-0 bg-3d-section bg-cover bg-center bg-no-repeat opacity-40" />
                )}
                ...
                <div className="relative z-10 max-w-5xl px-6 text-center">
                    ...
                    >
                        {(heroData as any)?.about_hero_tag || "Who we are"}
                    </motion.p>
